...
class MetaXTBFeed(DataBase.__class__):
    def __init__(cls, name, bases, dct):
        '''Class has already been created ... register'''
        # Initialize the class
        super(MetaXTBFeed, cls).__init__(name, bases, dct)

        # Register with the store
        XTBStore.DataCls = cls


class XTBFeed(with_metaclass(MetaXTBFeed, DataBase)):
    ...
    def _fetch_ohlcv(self, fromdate=None, todate=None):
        ...
        if fromdate and todate:
            since = int((fromdate - todate).total_seconds() * 1000)
        ...
        while True:
            dlen = len(self._data)
            data = sorted(self.store.fetch_ohlcv(self.p.dataname, 
    ...
    def _load_ohlcv(self):
        ...
        dtime = datetime.utcfromtimestamp(tstamp)
